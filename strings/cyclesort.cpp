...
vector<int> findDisappearedNumbers(vector<int> &v)
{
    ...
    while(i<n){
        int pos=i+1;
        if(v[i]<0){
            //transfered  at correct place
            i++;
            continue;
        }
        if(v[i]==pos){
            //naturally at correct place
            v[i]=v[i]*-1;
            i++;
            continue;
        }
        int target_index=v[i]-1;
        if(v[target_index]<0){
            //duplication occured
            i++;
            continue;
            ...
            //time to trnafer
            int temp=v[target_index];
            v[target_index]=v[i]*-1;
            v[i]=temp;
        }

    }
    for(int i=0;i<n;i++){
        //if a number is negative we know it its present
        //if a number is positive we know the index is missing
        if(v[i]>0){
            int value=i+1;
            res.push_back(value);
        }
