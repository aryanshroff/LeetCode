...
vector<int> findRedundantConnection(vector<vector<int>> &edges)
{
    ...
    //now we have a vector of hashsets 
    for(int i=0;i<n;i++){
        int parent=edges[i][0];
        int child=edges[i][1];
        m[parent].insert(child);
    }
    //we start removing an edge one by one from behind
    for(int i=n-1;i>=0;i--){
        int parent=edges[i][0];
        int child=edges[i][1];
        m[parent].erase(child);
        //call the topological check function here
        //execution ends add the child back
        m[parent].insert(child);
    }
}
