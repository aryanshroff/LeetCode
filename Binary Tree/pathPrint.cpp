...
public:
    void solve(TreeNode *root, string path, vector<string> &res)
    {
        ...
        s += to_string(val);
        path += s;
        // dont make recursive calls on null node untill its a leaf node
        if (root->left == NULL and root->right == NULL)
        {
            // since both children are null we are gonna make a call on any one of them
        ...
        // reached here means both non null
        else
        {
            solve(root->left, path, res);
            solve(root->right, path, res);
